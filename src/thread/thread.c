...
struct mthpc_barrier_object {
    ...
    atomic_long refcount; /* For cluster-level barrier only */
};
...
mthpc_group_put_barrier_object(struct mthpc_thread_group *group)
{
    if (READ_ONCE(group->barrier_object)) {
        long nr_thread =
            atomic_fetch_add_explicit(&group->barrier_object->refcount,
                                      -group->nr, memory_order_relaxed) -
            group->nr;
...
{
    atomic_fetch_add_explicit(&group->nr_exited, 1, memory_order_release);
}
...
void __mthpc_thread_run(void *threads, unsigned int nr)
{
    ...
    atomic_store_explicit(&barrier_object.refcount, total_nr_thread,
...
void __mthpc_thread_async_wait(void *threads, unsigned int nr)
{
    ...
    for (int i = 0; i < nr; i++) {
        ...
        for (int j = 0; j < group->nr; j++) {
            while (atomic_load_explicit(&group->nr_exited,
